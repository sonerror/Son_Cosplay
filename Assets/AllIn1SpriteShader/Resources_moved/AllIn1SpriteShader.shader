Shader "AllIn1SpriteShader/AllIn1SpriteShader"
{
    Properties
    {
		...
		_TextureScrollLoopXSpeed("Speed X Axis", Range(-5, 5)) = 1 //164
		_TextureScrollLoopYSpeed("Speed Y Axis", Range(-5, 5)) = 0 //165
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			#pragma shader_feature TEXTURESCROLLLOOP_ON
			...
			#if TEXTURESCROLLLOOP_ON
			half _TextureScrollLoopXSpeed, _TextureScrollLoopYSpeed;
			#endif
            ...
            half4 frag (v2f i) : SV_Target
            {
				...
				#if TEXTURESCROLLLOOP_ON && ATLAS_ON
				i.uv = half2(_MinXUV + ((_MaxXUV - _MinXUV) * (abs(((_Time.y + _RandomSeed) * _TextureScrollLoopXSpeed) + uvRect.x) % 1)),
				_MinYUV + ((_MaxYUV - _MinYUV) * (abs(((_Time.y + _RandomSeed) * _TextureScrollLoopYSpeed) + uvRect.y) % 1)));
				#endif
				...
				#if TEXTURESCROLLLOOP_ON && !ATLAS_ON
				i.uv.x += (sin(_Time.y + _RandomSeed) * _TextureScrollLoopXSpeed) % 1;
				i.uv.y += (sin(_Time.y + _RandomSeed) * _TextureScrollLoopYSpeed) % 1;
				#endif
