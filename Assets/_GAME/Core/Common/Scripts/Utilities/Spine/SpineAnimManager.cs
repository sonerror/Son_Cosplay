...
namespace Utilities.Spine
{
    ...
    public class SpineAnimManager : MonoBehaviour
    {
        ...
        public void PlayAnimGroup(AnimAssetGroupConfig animGroupConfig, float speed) => PlayAnimGroup(animGroupConfig, PlayAnimGroupMode.REPLACE, speed);
        public void PlayAnimGroup(AnimAssetGroupConfig animGroupConfig, PlayAnimGroupMode playMode = PlayAnimGroupMode.REPLACE, float speed = 1f)
        {
            if (animGroupConfig.animations == null || animGroupConfig.animations.Length == 0) return;
            ...
            foreach (AnimationReferenceAsset animAsset in animGroupConfig.animations)
            {
                if (_dictAnimAssetTrackIndex.TryGetValue(animAsset, out int trackIndex))
                {
                    if (!isTrackUsed[trackIndex])
                    {
                        ...
                        {
                            bool isLoop = false;
                            bool isFreezeAfterEnd = false;
                            if (_dictAnimAssetConfig.TryGetValue(animAsset, out var animAssetConfig))
                            {
                                isLoop = animAssetConfig.isLoop;
                                isFreezeAfterEnd = animAssetConfig.isFreezeAfterEnd;
                            }
                            ...
                            else
                            {
                                ...
                                if (!isFreezeAfterEnd) skeletonAnimation.AnimationState.AddEmptyAnimation(trackIndex, defaultMixDuration, 0f);
                            }
                        }
                    }
                }
            ...
            skeletonAnimation.timeScale = animGroupConfig.timeScale * speed;
